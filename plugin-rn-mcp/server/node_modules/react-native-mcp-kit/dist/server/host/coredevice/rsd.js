...
exports.fetchPeerInfo = void 0;
const node_net_1 = require("node:net");
const xpc_1 = require("./xpc");
...
const buildH2Frame = (type, flags, streamId, payload) => {
...
class PeerInfoParser {
    ...
    push(chunk) {
        ...
        while (this.offset + 16 <= this.buffer.length) {
            let parsed;
            try {
                parsed = (0, xpc_1.parseXpcWrapper)(this.buffer, this.offset);
            }
            catch {
...
// 1, close.
const fetchPeerInfo = async (deviceAddress, hostAddress, rsdPort, options = {}) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            socket = (0, node_net_1.createConnection)({ host: deviceAddress, localAddress: hostAddress, port: rsdPort });
        }
        catch (err) {
            reject(err);
            return;
        ...
        socket.on('connect', () => {
            ...
            socket.write(Buffer.concat([
                ...
                buildDataFrame(ROOT_CHANNEL_STREAM, (0, xpc_1.buildXpcWrapper)(xpc_1.XpcFlags.ALWAYS_SET, 0n, {})),
                buildEmptyHeadersFrame(REPLY_CHANNEL_STREAM),
                buildDataFrame(REPLY_CHANNEL_STREAM, (0, xpc_1.buildXpcWrapper)(xpc_1.XpcFlags.ALWAYS_SET | xpc_1.XpcFlags.INIT_HANDSHAKE, 0n, null)),
            ]));
        });
        socket.on('data', (chunk) => {
            ...
            while ((frame = reader.pop()) !== null) {
                ...
                if (frame.type === FRAME_TYPE.GOAWAY) {
                    const lastStream = frame.payload.length >= 4 ? frame.payload.readUInt32BE(0) & 0x7fffffff : 0;
                    const errorCode = frame.payload.length >= 8 ? frame.payload.readUInt32BE(4) : 0;
                    cleanup(new RsdHandshakeError(`RSD sent GOAWAY (last_stream=${lastStream}, error=${errorCode})`));
                    return;
                }
            }
        });
