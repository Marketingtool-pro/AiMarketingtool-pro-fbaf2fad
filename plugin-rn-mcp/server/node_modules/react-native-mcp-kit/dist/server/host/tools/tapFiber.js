...
On unmounted: { error: "fiber has no measurable host view", mcpId?, name }.`,
        handler: async (args, ctx) => {
            ...
            const clientId = args.clientId ?? ctx.requestedClientId;
            const index = args.index;
            const queryResult = await ctx.dispatch(`fiber_tree${protocol_1.MODULE_SEPARATOR}query`, { limit: 10, select: ['bounds', 'mcpId', 'name', 'testID'], steps }, clientId);
            if (!queryResult.ok) {
            ...
            const tapResult = await ctx.dispatch(`host${protocol_1.MODULE_SEPARATOR}tap`, { x: pick.bounds.centerX, y: pick.bounds.centerY }, clientId);
            if (!tapResult.ok) {
                return { error: `host__tap failed: ${tapResult.error}` };
            }
            return {
                bounds: pick.bounds,
                device: tapResult.result.device,
                mcpId: pick.mcpId,
                name: pick.name,
                tapped: true,
                testID: pick.testID,
...
exports.tapFiberTool = tapFiberTool;
//# sourceMappingURL=tapFiber.js.map