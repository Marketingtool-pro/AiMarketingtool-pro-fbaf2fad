...
exports.restartAppTool = exports.terminateAppTool = exports.launchAppTool = void 0;
...
const isAndroidPackageInstalled = async (serial, packageName, runner) => {
    try {
        const proc = await runner('adb', ['-s', serial, 'shell', 'pm', 'list', 'packages', packageName], { timeoutMs: PM_LIST_TIMEOUT_MS });
...
const resolveLaunchTarget = async (ctx, args, runner) => {
    ...
    const explicitAppId = (0, helpers_1.parseStringArg)(args.appId);
...
const APPID_SCHEMA = {
    description: 'iOS bundle ID or Android package name. Optional when a connected client already registered its bundleId.',
    examples: ['com.example.app', 'com.example.android'],
    type: 'string',
};
const launchAppTool = (runner) => {
    return {
...
exports.launchAppTool = launchAppTool;
const terminateAppTool = (runner) => {
    return {
...
exports.terminateAppTool = terminateAppTool;
const RESTART_TEARDOWN_DELAY_MS = 200;
const restartAppTool = (runner) => {
    return {
        description: 'Terminate then immediately relaunch an app (host__terminate_app + host__launch_app in one call, with a short teardown delay).',
        handler: async (args, ctx) => {
            const target = await resolveLaunchTarget(ctx, args, runner);
            if (!target.ok) {
                return { error: target.error };
            }
            const terminated = target.device.platform === 'ios'
                ? await terminateIos(target.device.nativeId, target.bundleId, runner)
                : await terminateAndroid(target.device.nativeId, target.bundleId, runner);
            ...
            const launched = target.device.platform === 'ios'
                ? await launchIos(target.device.nativeId, target.bundleId, runner)
                : await launchAndroid(target.device.nativeId, target.bundleId, runner);
...
exports.restartAppTool = restartAppTool;
