        ...
        tools: {
            ...
            query: {
                ...
                inputSchema: {
                    cache: {
                        default: true,
                        description: 'Reuse the match set when the React tree has not committed since the previous identical steps — detected via fiber root pointer equality. Pass false to force a fresh traversal.',
                        type: 'boolean',
                    },
                    dedup: {
                        default: true,
                        description: 'Drop wrapper cascades — a fiber is removed when any of its ancestors is also in the match set (PressableView → Pressable → View → RCTView collapses to the topmost). Independent siblings with overlapping bounds are kept. Pass false to keep every match.',
                        type: 'boolean',
                    },
                    limit: {
                        default: constants_1.QUERY_LIMIT_DEFAULT,
                        description: 'Max matches to return. truncated: true is added when total exceeds limit.',
                        maximum: constants_1.QUERY_LIMIT_MAX,
                        minimum: 1,
                        type: 'number',
                    },
                    onlyVisible: {
                    ...
                    waitFor: {
                        ...
                        properties: {
                            interval: {
                                default: constants_1.WAIT_INTERVAL_DEFAULT,
                                description: 'Gap between polls in milliseconds.',
                                minimum: constants_1.WAIT_INTERVAL_MIN,
                                type: 'number',
                            },
                            stable: {
                                default: 0,
                                description: 'Require the predicate to hold continuously for this many ms before returning.',
                                minimum: 0,
                                type: 'number',
                            },
                            timeout: {
                                default: constants_1.WAIT_TIMEOUT_DEFAULT,
                                description: 'Max wait in milliseconds.',
                                maximum: constants_1.WAIT_TIMEOUT_MAX,
                                minimum: 1,
                                type: 'number',
                            },
                            until: {
