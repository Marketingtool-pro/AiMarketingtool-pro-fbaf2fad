...
exports.findAllByQuery = exports.matchesQuery = void 0;
 ...
 */
const stringifyForMatching = (value) => {
...
const matchStringCriterion = (actual, pattern, mode) => {
    if (actual === undefined)
        return false;
...
const matchesQuery = (fiber, query) => {
    try {
        if (query.mcpId &&
            !matchStringCriterion(fiber.memoizedProps?.['data-mcp-id'], query.mcpId, 'equals')) {
            return false;
        }
        if (query.testID &&
            !matchStringCriterion(fiber.memoizedProps?.testID, query.testID, 'equals')) {
            return false;
        }
        if (query.name && !matchStringCriterion((0, naming_1.getComponentName)(fiber), query.name, 'equals')) {
            return false;
        }
        if (query.text) {
            const content = (0, serialize_1.getTextContent)(fiber);
            if (!matchStringCriterion(content ?? undefined, query.text, 'includes'))
                return false;
        }
        ...
        if (query.props && typeof query.props === 'object') {
            ...
            for (const key of Object.keys(query.props)) {
                if (!matchPropValue(props[key], query.props[key]))
                    return false;
            }
        }
        if (Array.isArray(query.any) && query.any.length > 0) {
            const anyMatched = query.any.some((sub) => {
                return (0, exports.matchesQuery)(fiber, sub);
            });
            if (!anyMatched)
                return false;
        }
        if (query.not) {
            if (Array.isArray(query.not)) {
                for (const sub of query.not) {
                    if ((0, exports.matchesQuery)(fiber, sub))
                        return false;
                }
            }
            else if (typeof query.not === 'object') {
                if ((0, exports.matchesQuery)(fiber, query.not))
                    return false;
            }
        }
        return true;
...
const findAllByQuery = (root, query) => {
    return (0, traverse_1.findAllFibers)(root, (fiber) => {
        return (0, exports.matchesQuery)(fiber, query);
    });
};
