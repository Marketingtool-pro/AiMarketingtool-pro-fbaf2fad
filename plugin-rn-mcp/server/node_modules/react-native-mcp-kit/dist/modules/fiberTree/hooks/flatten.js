...
exports.flatHooksToTree = exports.flattenHookMeta = void 0;
...
const flattenHookMeta = (meta, via = [], seen = new WeakSet(), maxDepth = Infinity) => {
    ...
    for (const entry of meta) {
        ...
        if (sub && !seen.has(fn) && via.length < maxDepth) {
            ...
            out.push(...(0, exports.flattenHookMeta)(sub, [...via, entry.name], seen, maxDepth));
            seen.delete(fn);
        }
        else {
            out.push({ ...entry, via });
...
exports.flattenHookMeta = flattenHookMeta;
...
const flatHooksToTree = (flat) => {
    ...
    for (const entry of flat) {
        ...
        if (entry.mcpId !== undefined)
            node.mcpId = entry.mcpId;
...
exports.flatHooksToTree = flatHooksToTree;
