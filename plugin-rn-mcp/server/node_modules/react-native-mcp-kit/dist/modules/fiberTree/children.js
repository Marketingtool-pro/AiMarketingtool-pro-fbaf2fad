...
exports.walkChildren = exports.parseChildrenOptions = exports.parseChildrenSelect = void 0;
 ...
 */
const parseChildrenSelect = (selectArg) => {
        ...
        for (const entry of selectArg) {
            if (typeof entry === 'string') {
                if (entry === 'props' || entry === 'hooks') {
                    throw new Error(`select.children.select cannot include "${entry}" — heavy fields are not supported inside the walker. Run a second query({ steps: [{ mcpId: '<child-mcpId>' }], select: ['${entry}'] }) on the specific node instead.`);
                }
                if (!LIGHT_FIELDS.has(entry) && entry !== 'children') {
                    throw new Error(`select.children.select: unknown field "${entry}". Allowed: mcpId / name / testID / bounds / { children }`);
                }
                if (entry !== 'children')
                    fields.add(entry);
                continue;
            ...
            if (entry && typeof entry === 'object') {
                for (const [key, value] of Object.entries(entry)) {
                    ...
                    if (key === 'props' || key === 'hooks') {
                        throw new Error(`select.children.select cannot include "${key}" — heavy fields are not supported inside the walker. Run a second query({ steps: [{ mcpId: '<child-mcpId>' }], select: ['${key}'] }) on the specific node instead.`);
                    }
                    if (key === 'children') {
                        nestedChildren = (0, exports.parseChildrenOptions)(value);
                        continue;
                    }
                    if (!LIGHT_FIELDS.has(key)) {
                        throw new Error(`select.children.select: unknown field "${key}". Allowed: mcpId / name / testID / bounds / { children }`);
                    }
                    fields.add(key);
                }
            }
...
const parseChildrenOptions = (raw) => {
    ...
    if (raw && typeof raw === 'object') {
        const obj = raw;
        const treeDepth = typeof obj.treeDepth === 'number' && obj.treeDepth >= 0
            ? clampTreeDepth(obj.treeDepth)
            : CHILDREN_DEFAULT_TREE_DEPTH;
        const itemsCap = typeof obj.itemsCap === 'number' && obj.itemsCap >= 0
            ? Math.floor(obj.itemsCap)
            : CHILDREN_DEFAULT_ITEMS_CAP;
        const select = (0, exports.parseChildrenSelect)(obj.select);
...
exports.parseChildrenOptions = parseChildrenOptions;
const clampTreeDepth = (n) => {
    if (!Number.isFinite(n) || n < 0)
        return CHILDREN_DEFAULT_TREE_DEPTH;
 ...
 */
const projectChildLightFields = async (fiber, fields, measure) => {
    const out = {};
    if (fields.has('mcpId')) {
        out.mcpId = fiber.memoizedProps?.['data-mcp-id'];
    }
    if (fields.has('name')) {
        out.name = (0, utils_1.getComponentName)(fiber);
    }
 ...
 */
const walkChildren = async (fiber, options, depthLeft, measure) => {
    if (depthLeft <= 0)
        return [];
    const kids = (0, utils_1.getDirectChildren)(fiber);
    ...
    for (const kid of sliced) {
        ...
        if (depthLeft > 1) {
            node.children = await (0, exports.walkChildren)(kid, nextOptions, depthLeft - 1, measure);
        }
        else {
            ...
            const subCount = (0, utils_1.getDirectChildren)(kid).length;
            if (subCount > 0) {
