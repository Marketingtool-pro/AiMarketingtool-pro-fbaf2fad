...
exports.reactQueryModule = void 0;
...
const QUERIES_SCHEMA = (0, projectValue_1.makeProjectionSchema)(QUERIES_DEFAULT_DEPTH);
const DATA_SCHEMA = (0, projectValue_1.makeProjectionSchema)(DATA_DEFAULT_DEPTH);
...
const reactQueryModule = (queryClient) => {
    const getAllQueries = () => {
        ...
        tools: {
            get_data: {
                ...
                handler: (args) => {
                    ...
                    if (!query)
                        return { error: `Query with key ${args.key} not found` };
                    const result = {
                    ...
                    return (0, projectValue_1.applyProjection)(result, args, projectValue_1.projectAsValue, DATA_DEFAULT_DEPTH);
                },
                inputSchema: {
                    ...DATA_SCHEMA,
                    key: {
                ...
                handler: (args) => {
                    ...
                    if (args.status) {
                        queries = queries.filter((q) => {
                            return q.state.status === args.status;
                        });
                    }
                    if (args.key) {
                    ...
                    return (0, projectValue_1.applyProjection)(queries.map(serializeQuery), args, projectValue_1.projectAsValue, QUERIES_DEFAULT_DEPTH);
                },
                inputSchema: {
                    ...QUERIES_SCHEMA,
            ...
            mutate: {
                description: 'Mutate the cache: `invalidate` marks queries stale (refetches on next use); `refetch` refetches immediately; `remove` drops queries entirely; `reset` clears data + error back to initial state. Omit `key` to target every cached query.',
                handler: async (args) => {
...
exports.reactQueryModule = reactQueryModule;
