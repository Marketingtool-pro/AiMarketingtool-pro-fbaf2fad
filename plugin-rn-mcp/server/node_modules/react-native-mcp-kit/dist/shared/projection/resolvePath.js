...
exports.resolvePath = exports.parsePath = void 0;
const parsePath = (path) => {
    ...
    while (i < n) {
        ...
        if (ch === '[') {
            ...
            if (closeIdx === -1)
                throw new Error(`Unbalanced bracket at position ${i}`);
        ...
        if (name === '')
            throw new Error(`Empty key at position ${i}`);
...
const findMatchingBracket = (path, openIdx) => {
    let inSingle = false;
    let inDouble = false;
...
const parseBracketContents = (inner) => {
...
const resolvePath = (root, path) => {
    let segments;
    try {
        segments = (0, exports.parsePath)(path);
    }
    catch (e) {
        return {
            actual: root,
...
const segmentSeparator = (seg) => {
    return seg.kind === 'key' ? '.' : '';
};
const segmentToString = (seg) => {
    switch (seg.kind) {
        case 'key':
...
const stepIndex = (current, index) => {
    ...
    if (current && typeof current === 'object') {
        ...
        return { ok: true, value: current[keys[real]] };
    }
    return { error: `Cannot index non-container with [${index}]`, ok: false };
};
const stepSlice = (current, seg) => {
    const { end, start } = seg;
    if (Array.isArray(current)) {
